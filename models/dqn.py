...
class DQN:
    def __init__(self, env: baseenv.BaseEnv, epsilon = 1.0, max_history = 50):
        ...
        self.epsilon = epsilon # Currently unused
        ...
        self.model, self.optim, _ = models.construct_densenet(self.features, self.actions, lr=0.005)
    ...
    def train(self, gamma = 0.8, num_epochs=1, num_episodes_per_learning_session=10, session_limit=5,
        ...
        for epoch in range(num_epochs):
            ...
            while num_sessions < session_limit and not done:
                ...
                #action = self.get_action(state, e=self.epsilon,
                #                         action_mask=valid_actions_mask, action_list=valid_actions)[0]
                action = self.get_action(state, e=self.epsilon)[0]
            ...
            print(f'Epoch {epoch} Loss: {(loss / samples):.3f} E: {curr_epsilon:.3f} G: {gamma:.3f} '+
                  f'Rewards: {rewards:.1f} Eval Rewards: {eval_score:.3f}')
